      ...
      if(isset($_POST['user'])){ 
        ...
        foreach ($_POST['user'] as $key => $value) { //remplissage de la variable filtreuser en fonction du nombre de user trouvé
            ...
            if((isset($_POST['possede']))and (!isset($_POST['fini']))){
                foreach ($_POST['user'] as $key => $value) { //remplissage de la variable filtreuser en fonction du nombre de user trouvé
                    if ($key<1){
                    $_SESSION['boubou'] = 'SELECT J.* ,M.idjeux ,M.idusers ,M.possede ,M.fini FROM jeux AS J, maintable AS M WHERE M.idusers='.$value.' AND J.id=M.idjeux AND M.possede="oui"';
                    $_SESSION['filtreuser'] = $_SESSION['boubou'];
                    }else {
            ...
            if((isset($_POST['fini']))and (!isset($_POST['possede']))){
                foreach ($_POST['user'] as $key => $value) { //remplissage de la variable filtreuser en fonction du nombre de user trouvé
                    if ($key<1){
                    $_SESSION['boubou'] = 'SELECT J.* ,M.idjeux ,M.idusers ,M.possede ,M.fini FROM jeux AS J, maintable AS M WHERE M.idusers='.$value.' AND J.id=M.idjeux AND M.fini="oui"';
                    $_SESSION['filtreuser'] = $_SESSION['boubou'];
                    }else {
