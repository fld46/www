...
class hawkiqPSApi {
	...
	$stats[3] =  $lastgame;
	    //$hiddensrc = $html->find('div[id=hidden', 0)->title;
	...
	//	}
//	}
	$info = array("username"=>$username,"avatar"=>$avatar,"total"=>(int)$total,"platinum"=>(int)$platinum,"gold"=>(int)$gold,"silver"=>(int)$silver,"bronze"=>(int)$bronze,"level"=>$level,"lastgame"=>$stats[3],"played"=>$stats[0],"completion"=>$stats[2],"complgames"=>$stats[1]);
	return $info;
}
...
public function get_trophiesGame($liens){
        $url = $liens;
        $html = file_get_html($url);
        $platinum = $html->find('li[class=icon-sprite platinum]',0)->plaintext;
        $gold = $html->find('li[class=icon-sprite gold]',0)->plaintext;
        ...
        $silver = str_replace($searchfor, '', $silver);
        $bronze = str_replace($searchfor, '', $bronze);
        $trophies = array("platinum"=>(int)$platinum,"gold"=>(int)$gold,"silver"=>(int)$silver,"bronze"=>(int)$bronze);
        $trophiest=(int)$platinum+(int)$gold+(int)$silver+(int)$bronze;
        return $trophies;
    }
public function get_Game($liens){
        $url = $liens;
        $html = file_get_html($url);
        $gameplatine = $html->find('li[niveau=platine]');
        $gamegold = $html->find('li[niveau=gold]');
        $gamesilver = $html->find('li[niveau=argent]');
        $gamebronze = $html->find('li[niveau=bronze]');
        $test=array();
        foreach($gameplatine as $e) {
         $test["platine"]=$e->name;   
       
        }
        foreach($gamegold as $e) {
         $test["or"][]=$e->name;   
       
        }
        foreach($gamesilver as $e) {
         $test["silver"][]=$e->name;   
       
        }
        foreach($gamebronze as $e) {
         $test["bronze"][]=$e->name;   
       
        }
        //$searchfor = array(",","%");
        //$game = str_replace($searchfor, '', $game);
        //$gold = str_replace($searchfor, '', $gold);
        //$silver = str_replace($searchfor, '', $silver);
        //$bronze = str_replace($searchfor, '', $bronze);
        //$trophies = array("platinum"=>(int)$platinum,"gold"=>(int)$gold,"silver"=>(int)$silver,"bronze"=>(int)$bronze);
