...
class Accountstat {
    ...
    public function getNbjeuxPossede(){
        $req = $this->_db->query('SELECT DISTINCT J.id FROM jeux AS J,maintable AS M WHERE  M.idjeux=J.id AND M.idusers= ? and M.possede="oui"',[$this->user_id]);
        $possede=$req->rowCount();
        return $possede;
                
    }
    public function getNbjeuxFini(){
        $req = $this->_db->query('SELECT DISTINCT J.id FROM jeux AS J,maintable AS M WHERE  M.idjeux=J.id AND M.idusers= ? and M.fini="oui"',[$this->user_id]);
        $fini=$req->rowCount();
        return $fini;
                
    }
    public function getNbjeuxSuggere(){
        //$req = $this->_db->query('SELECT DISTINCT J.id FROM jeux AS J,maintable AS M WHERE  M.idjeux=J.id AND M.idusers= ? AND M.possede="oui" AND J.difficulte!=0 AND J.difficulte IS NOT NULL AND (M.fini!="oui"  OR M.fini IS NULL) ORDER BY J.difficulte,J.temps ',[$this->user_id])->fetch();
        //if($req!=NULL){ 
        //$req2= $this->_db->query('SELECT * FROM jeux WHERE id = ?',[$req->id]);
        //while ($donnees = $req2->fetch(PDO::FETCH_ASSOC))
        $req = $this->_db->query('SELECT DISTINCT J.id FROM jeux AS J,maintable AS M WHERE  M.idjeux=J.id AND M.idusers= ? AND M.possede="oui" AND J.difficulte!=0 AND J.difficulte IS NOT NULL AND (M.fini!="oui"  OR M.fini IS NULL) ORDER BY J.difficulte,J.temps,J.titre ',[$this->user_id]);
        
                
        $nb=1;
        while ($donnees = $req->fetch(PDO::FETCH_ASSOC))
        {
            $reqfini = $this->_db->query('SELECT * FROM maintable  WHERE  idjeux= ? AND idusers= ? AND possede="oui" AND fini="oui"' ,[$donnees['id'],$this->user_id]);
            $req2= $this->_db->query('SELECT * FROM jeux WHERE id = ? ',[$donnees['id']]); 
            while (($reqfini->fetch()==NULL) and $donnees2 = $req2->fetch(PDO::FETCH_ASSOC) and $nb=1){
            $nb++;
        {
        $jeux = new Jeux();
        $jeux->hydrate($donnees2);
        } 
    ...
    public function getNbjeuxSuggeresuivant(){
        ...
        while ($donnees = $req->fetch(PDO::FETCH_ASSOC))
        {
            ...
            while (($reqfini->fetch()==NULL) and $donnees2 = $req2->fetch(PDO::FETCH_ASSOC) and $nb<10){
            $nb++;
