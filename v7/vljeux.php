...
{
    $pdo = new PDO('mysql:host=localhost;dbname=sitejeuxv3', 'root', 'et2tcmdp');
    $req = $pdo->prepare('SELECT DISTINCT J.id FROM jeux AS J,maintable AS M WHERE  M.idjeux=J.id AND M.idusers= ? AND M.possede="oui"   ORDER BY J.titre ');
...
while ($row1 = $req->fetch())
                ...
                {
                $req2 = $pdo->prepare('SELECT * FROM jeux  WHERE  id = ?');
                $req2->execute([$row1['id']]);
                ...
                $testfini=$pdo->prepare('SELECT * FROM maintable  WHERE  idjeux = ? AND idusers = ? AND fini ="oui"');
                $testfini->execute([$row["id"],$_SESSION['auth']->id]);
                if($testfini->rowcount()!=0){
                 $fill=true;   
                }else{
                 $fill=false;   
                }
